...
// add classes to ul sub-menus
  function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output )
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		$out_div = ($depth == 0 ? '<div class="second"><div class="inner"><div class="inner_arrow"></div><div class="inner2"><div class="inner2a"><ul>' : '');
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth, $args ) {
		...
		// build html
		$output .= $indent . '<li id="nav-menu-item-'. $item->ID . '" class="' . $class_names . ' ' . $active . ' ' . $first . '">';
  ...
  function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output )
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if (isset($_SESSION['qode_animation'])) $qode_animation = $_SESSION['qode_animation'];
		// depth dependent classes
		if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && (($qode_options_lounge['page_transitions'] == "0") || ($qode_animation == "no"))):
  ...
  function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output )
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		//number of sub items in one column
		if($qode_options_lounge['separation_number'] != ""):
			$separation_number = $qode_options_lounge['separation_number'];
		else:
			$separation_number = 6;
		endif;
